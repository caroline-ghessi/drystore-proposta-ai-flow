-- Corrigir função calcular_por_mapeamento para deduplicar dados e evitar inconsistências
-- que causam o problema de "piscamento" na interface
  ...
  calculos AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN pu.produto_categoria = 'STARTER_SHINGLE' OR (pu.tipo_calculo = 'customizado' AND pu.item_codigo = '10471') THEN v_perimetro_telhado
        -- Para CAP DE CUMEEIRA na cumeeira
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND pu.tipo_calculo = 'customizado' AND v_comprimento_cumeeira > 0 THEN v_comprimento_cumeeira
        -- Para CAP DE CUMEEIRA no espigão
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND pu.tipo_calculo = 'customizado' AND v_comprimento_espigao > 0 THEN v_comprimento_espigao
        -- Para FITA AUTOADESIVA na água furtada
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '15600' AND pu.tipo_calculo = 'customizado' THEN v_comprimento_agua_furtada
      ...
      CASE 
        ...
        WHEN pu.produto_categoria = 'STARTER_SHINGLE' AND v_perimetro_telhado > 0 THEN 
          CEILING(v_perimetro_telhado / 23)
        -- Cálculo customizado para CAP DE CUMEEIRA baseado na cumeeira
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND v_comprimento_cumeeira > 0 THEN 
          CEILING(v_comprimento_cumeeira / 5)
        -- Cálculo customizado para CAP DE CUMEEIRA baseado no espigão
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND v_comprimento_espigao > 0 THEN 
          CEILING(v_comprimento_espigao / 5)
        -- Cálculo customizado para FITA AUTOADESIVA baseado na água furtada
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '15600' AND v_comprimento_agua_furtada > 0 THEN 
      ...
      CASE 
        ...
        WHEN (pu.produto_categoria = 'STARTER_SHINGLE' AND v_perimetro_telhado > 0) OR 
             (pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND (v_comprimento_cumeeira > 0 OR v_comprimento_espigao > 0)) OR
             (pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '15600' AND v_comprimento_agua_furtada > 0) THEN 
          CASE 
            WHEN pu.produto_categoria = 'STARTER_SHINGLE' THEN CEILING(v_perimetro_telhado / 23)
            WHEN pu.item_codigo = '10472' AND v_comprimento_cumeeira > 0 THEN CEILING(v_comprimento_cumeeira / 5)
            WHEN pu.item_codigo = '10472' AND v_comprimento_espigao > 0 THEN CEILING(v_comprimento_espigao / 5)
      ...
      CASE 
        ...
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND v_comprimento_cumeeira > 0 THEN 
          CEILING(v_comprimento_cumeeira / 5) * pu.preco_unitario
        -- Valor para CAP DE CUMEEIRA (espigão)
        WHEN pu.produto_categoria = 'ACESSORIOS_SHINGLE' AND pu.item_codigo = '10472' AND v_comprimento_espigao > 0 THEN 
          CEILING(v_comprimento_espigao / 5) * pu.preco_unitario
