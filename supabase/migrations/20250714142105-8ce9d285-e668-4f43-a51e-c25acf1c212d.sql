...
-- Inserir configurações padrão para todos os tipos se não existirem
INSERT INTO layout_configuracoes (tipo_proposta, nome, descricao, configuracao_padrao, configuracao)
SELECT tipo, 
       'Layout Padrão ' || INITCAP(tipo),
       'Configuração padrão para propostas de ' || tipo,
       true,
       '{
         "header": {
           "titulo": "Proposta de ' || INITCAP(tipo) || '",
           "subtitulo": "Solução personalizada para suas necessidades",
           "mostrar_logo": true,
           "cor_fundo": "primary"
         },
         "credibilidade": {
...
WHERE NOT EXISTS (
  SELECT 1 FROM layout_configuracoes 
  WHERE tipo_proposta = t.tipo AND configuracao_padrao = true
);