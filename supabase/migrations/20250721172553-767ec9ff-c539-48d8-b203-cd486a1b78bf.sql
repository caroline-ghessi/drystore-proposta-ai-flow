...
-- 1. Criar o produto Fita Autoadesiva para Água Furtada
INSERT INTO produtos_mestre (
    ...
    '15600',
    'Fita Autoadesiva para Água Furtada',
    'ACESSORIOS_SHINGLE',
    'rl',
    395.90,
    0.9,
    5.0,
    'Impermeabilização de água furtada em telhados shingle',
    true
);

-- 2. Adicionar a Fita Autoadesiva à composição Telhas Shingle Supreme (1.16)
INSERT INTO itens_composicao (
    ...
    pm.id as produto_id,
    1.11 as consumo_por_m2, -- 1/0.9 = 1.11 m de produto por metro linear
    5.0 as quebra_aplicada,
    1.0 as fator_correcao,
    (pm.preco_unitario / pm.quantidade_embalagem) as valor_unitario,
    (pm.preco_unitario / pm.quantidade_embalagem) * 1.11 * 1.05 as valor_por_m2,
    20 as ordem,
    'customizado' as tipo_calculo,
    'Cálculo baseado em comprimento de água furtada' as observacoes_calculo
FROM composicoes_mestre cm
CROSS JOIN produtos_mestre pm
WHERE cm.codigo = '1.16' 
AND pm.codigo = '15600';

-- 3. Adicionar a Fita Autoadesiva à composição Telhas Shingle Oakridge (1.17)
INSERT INTO itens_composicao (
    composicao_id,
    produto_id,
    consumo_por_m2,
...
-- 4. Atualizar os valores totais das composições
...
-- 5. Atualizar função calcular_por_mapeamento para suportar espigão e água furtada
