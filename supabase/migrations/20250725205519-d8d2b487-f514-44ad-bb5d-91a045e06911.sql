-- Atualizar função calcular_por_mapeamento para arredondar quantidades exceto SUBCOBERTURA TYVEK (código 7427)
  ...
  calculos AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN pu.item_codigo = '5177' AND v_perimetro_telhado > 0 THEN 
          (v_perimetro_telhado / 23) * (1 + pu.quebra_aplicada/100)
        -- PREGO ROLO SHINGLE: usar mesma base da telha
        WHEN pu.item_codigo = '3539' THEN 
      ...
      pu.preco_unitario,
      CASE 
        -- PARA TODOS OS PRODUTOS EXCETO SUBCOBERTURA TYVEK: ARREDONDAR PARA CIMA
        ...
        WHEN pu.item_codigo = '5177' AND v_perimetro_telhado > 0 THEN 
          CEIL((v_perimetro_telhado / 23) * (1 + pu.quebra_aplicada/100)) * pu.preco_unitario
        -- PREGO ROLO SHINGLE: valor baseado na quantidade de rolos
        WHEN pu.item_codigo = '3539' THEN 
        ...
        WHEN pu.item_codigo = '10420' THEN 
          CEIL((v_area_calculada / 3.1) * (1 + pu.quebra_aplicada/100)) * pu.preco_unitario
        -- SUBCOBERTURA TYVEK PROTEC 120: EXCEÇÃO - Manter fração exata para compra
        WHEN pu.item_codigo = '7427' THEN 
          (CASE 
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 30.96 THEN 1.0/3 -- 1/3 de rolo
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 61.92 THEN 2.0/3 -- 2/3 de rolo
        ...
        -- Valor padrão com arredondamento para cima
