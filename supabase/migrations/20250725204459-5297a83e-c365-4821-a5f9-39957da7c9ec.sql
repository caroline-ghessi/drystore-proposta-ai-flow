-- Atualizar função calcular_por_mapeamento para corrigir cálculo do produto 3539 (PREGO ROLO SHINGLE)
-- O produto deve ter a mesma quantidade que os pacotes de telha (1:1)
...
DECLARE
  ...
  v_incluir_cumeeira_ventilada BOOLEAN;
BEGIN
  ...
  v_incluir_cumeeira_ventilada := COALESCE((p_dados_extras->>'incluir_cumeeira_ventilada')::boolean, false);
  ...
  produtos_unicos AS (
    ...
    AND (
      ...
      OR
      -- NOVA LÓGICA: Incluir CUMEEIRA VENTILADA apenas se:
      -- 1. Houver cumeeira OU espigão E
      -- 2. O usuário optou por incluir cumeeira ventilada
      (pm.codigo = '5298' AND v_incluir_cumeeira_ventilada = true AND (v_comprimento_cumeeira > 0 OR v_comprimento_espigao > 0))
      OR
  ...
  calculos AS (
    SELECT 
      ...
      CASE 
        ...
        -- Para PREGO ROLO SHINGLE (código 3539), usar área (igual à telha)
        ...
        -- Para SUBCOBERTURA TYVEK PROTEC 120 (código 7427), usar área
        WHEN pu.item_codigo = '7427' THEN v_area_calculada
      ...
      CASE 
        ...
        WHEN pu.item_codigo = '5177' AND v_perimetro_telhado > 0 THEN 
          v_perimetro_telhado / 23
        -- PREGO ROLO SHINGLE: área ÷ 3.1 (1 rolo por pacote de telha) - CORRIGIDO
        ...
        WHEN pu.item_codigo = '10420' THEN 
          v_area_calculada / 3.1
        -- SUBCOBERTURA TYVEK PROTEC 120 (código 7427): área ÷ 92.9 com lógica de fracionamento
        WHEN pu.item_codigo = '7427' THEN 
          v_area_calculada / 92.9
      ...
      CASE 
        ...
        WHEN pu.item_codigo = '5177' AND v_perimetro_telhado > 0 THEN 
          (v_perimetro_telhado / 23) * (1 + pu.quebra_aplicada/100)
        -- PREGO ROLO SHINGLE: usar mesma base da telha - CORRIGIDO
        WHEN pu.item_codigo = '3539' THEN 
        ...
        WHEN pu.item_codigo = '10420' THEN 
          (v_area_calculada / 3.1) * (1 + pu.quebra_aplicada/100)
        -- SUBCOBERTURA TYVEK PROTEC 120: aplicar quebra e lógica de fracionamento
        WHEN pu.item_codigo = '7427' THEN 
          -- Calcular área com quebra
          CASE 
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 30.96 THEN 1.0/3 -- 1/3 de rolo
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 61.92 THEN 2.0/3 -- 2/3 de rolo
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 92.9 THEN 1.0 -- 1 rolo inteiro
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 123.86 THEN 4.0/3 -- 1 rolo + 1/3
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 154.82 THEN 5.0/3 -- 1 rolo + 2/3
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 185.8 THEN 2.0 -- 2 rolos
            ELSE CEIL((v_area_calculada * (1 + pu.quebra_aplicada/100)) / 92.9 * 3) / 3.0 -- Múltiplos de 1/3
          END
      ...
      CASE 
        ...
        WHEN pu.item_codigo = '5177' AND v_perimetro_telhado > 0 THEN 
          CEIL((v_perimetro_telhado / 23) * (1 + pu.quebra_aplicada/100)) * pu.preco_unitario
        -- PREGO ROLO SHINGLE: valor baseado na quantidade de rolos - CORRIGIDO
        WHEN pu.item_codigo = '3539' THEN 
        ...
        WHEN pu.item_codigo = '10420' THEN 
          CEIL((v_area_calculada / 3.1) * (1 + pu.quebra_aplicada/100)) * pu.preco_unitario
        -- SUBCOBERTURA TYVEK PROTEC 120: valor baseado na fração calculada
        WHEN pu.item_codigo = '7427' THEN 
          (CASE 
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 30.96 THEN 1.0/3 -- 1/3 de rolo
            WHEN (v_area_calculada * (1 + pu.quebra_aplicada/100)) <= 61.92 THEN 2.0/3 -- 2/3 de rolo
