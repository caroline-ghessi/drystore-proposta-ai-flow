...
export interface ParametrosDrywallHibrido {
  largura: number;
  altura: number;
  tipo_parede?: string;
...
export interface ItemCalculoDrywallHibrido {
  categoria: string;
  item_codigo: string;
...
export interface ResumoDrywallHibrido {
  valorTotal: number;
  valorPorM2: number;
  pesoTotal: number;
  areaLiquida: number;
...
export const useDrywallCalculosHibrido = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const { toast } = useToast();

  const calcularOrcamentoDrywallHibrido = async (
    parametros: ParametrosDrywallHibrido
  ): Promise<{
    itens: ItemCalculoDrywallHibrido[];
    resumo: ResumoDrywallHibrido;
  } | null> => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const resumo: ResumoDrywallHibrido = {
        valorTotal,
      ...
      toast({
        ...
        description: `Orçamento calculado: R$ ${valorTotal.toFixed(2)} (R$ ${(valorTotal / areaLiquida).toFixed(2)}/m²)`,
      });
      ...
      console.error('Erro ao calcular orçamento drywall híbrido:', error);
  ...
  const validarExemploGuiaHibrido = async (): Promise<boolean> => {
    // Validar com exemplo do guia: 6m × 3m com porta e janela
    const parametrosExemplo: ParametrosDrywallHibrido = {
      largura: 6,
      altura: 3,
      incluir_portas: true,
      quantidade_portas: 1,
    ...
    const resultado = await calcularOrcamentoDrywallHibrido(parametrosExemplo);
    ...
    toast({
      title: "✅ Validação concluída - Sistema Híbrido",
      description: `Exemplo validado! Valor: R$ ${resumo.valorTotal.toFixed(2)} (R$ ${resumo.valorPorM2.toFixed(2)}/m²). Produtos controlados via admin.`,
    });
  ...
  return {
    calcularOrcamentoDrywallHibrido,
    validarExemploGuiaHibrido,
    loading,
    error
  };
};