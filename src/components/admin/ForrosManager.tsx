...
export const ForrosManager = () => {
  const [produtos, setProdutos] = useState<ProdutoForro[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [editingProduto, setEditingProduto] = useState<ProdutoForro | null>(null);
  ...
  const fetchProdutos = async () => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Erro ao carregar produtos de forros",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async (produtoData: Partial<ProdutoForro>) => {
    try {
      const dataToSave = {
        ...produtoData,
        categoria: 'forros'
  ...
  const toggleStatus = async (produto: ProdutoForro) => {
    try {
      const { error } = await supabase
        .from('produtos')
  ...
  const openEditDialog = (produto?: ProdutoForro) => {
  ...
  const exportarCSV = () => {
    ...
    a.href = url;
    a.download = 'produtos-forros.csv';
    a.click();
  };

  if (loading) {
    return <div className="p-6">Carregando produtos de forros...</div>;
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle>Gestão de Forros</CardTitle>
        <CardDescription>
          Gerencie o catálogo de produtos de forros
        </CardDescription>
      </CardHeader>
...
interface ProdutoEditDialogProps {
  produto: ProdutoForro | null;
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Partial<ProdutoForro>) => void;
}

const ProdutoEditDialog = ({ produto, isOpen, onClose, onSave }: ProdutoEditDialogProps) => {
  const [formData, setFormData] = useState<Partial<ProdutoForro>>({
    categoria: 'forros',
    nome: '',
    fabricante: '',
    preco_unitario: 0,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {produto ? 'Editar Produto' : 'Novo Produto de Forro'}
          </DialogTitle>
        </DialogHeader>
