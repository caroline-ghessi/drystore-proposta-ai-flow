...
export const PisosManager = () => {
  const [produtos, setProdutos] = useState<ProdutoPiso[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [editingProduto, setEditingProduto] = useState<ProdutoPiso | null>(null);
  ...
  const fetchProdutos = async () => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Erro ao carregar produtos de pisos",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async (produtoData: Partial<ProdutoPiso>) => {
    try {
      const dataToSave = {
        ...produtoData,
        categoria: 'pisos'
  ...
  const toggleStatus = async (produto: ProdutoPiso) => {
    try {
      const { error } = await supabase
        .from('produtos')
  ...
  const openEditDialog = (produto?: ProdutoPiso) => {
  ...
  const exportarCSV = () => {
    ...
    a.href = url;
    a.download = 'produtos-pisos.csv';
    a.click();
  };

  if (loading) {
    return <div className="p-6">Carregando produtos de pisos...</div>;
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <span>Gestão de Pisos</span>
        </CardTitle>
        <CardDescription>
          Gerencie o catálogo de produtos de pisos
        </CardDescription>
      </CardHeader>
...
interface ProdutoEditDialogProps {
  produto: ProdutoPiso | null;
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Partial<ProdutoPiso>) => void;
}

const ProdutoEditDialog = ({ produto, isOpen, onClose, onSave }: ProdutoEditDialogProps) => {
  const [formData, setFormData] = useState<Partial<ProdutoPiso>>({
    categoria: 'pisos',
    nome: '',
    fabricante: '',
    preco_unitario: 0,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {produto ? 'Editar Produto' : 'Novo Produto de Piso'}
          </DialogTitle>
        </DialogHeader>
